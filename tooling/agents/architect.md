...
- [ ] **Simplifications list lives in `ir/simplifications.yaml`, NOT `ir/game_ir.yaml`.** When you document that this slice closes a target-vs-as-is delta (by removing an entry and replacing it with a `# Removed: ...` comment naming the slice) OR adds a newly-noticed deferral, edit `ir/simplifications.yaml`. Do NOT touch `ir/game_ir.yaml`'s body for this kind of bookkeeping. Rationale: `ir/game_ir.yaml` is in `tooling/orchestrator_run.py § FROZEN_CONTEXT_FILES` and in `tooling/partial_regen.py § global` triggers — any edit forces partial-scope PRs (which touch 1-2 modules) into full 14-module regens, multiplying token cost ~7x for what is usually a 5-10 line narrative update. `ir/simplifications.yaml` is deliberately NOT in either list. See the file's header docstring for the lifecycle convention. Only edit `ir/game_ir.yaml`'s body when you change the actual target shape (combat constants, archetype tables, etc.) — those edits SHOULD trigger global regen because the prompt cache key really did change.
- [ ] **Cross-shard freshness.** When a spec edit changes a value listed in any `ir/contracts/<module>.yaml § public_constants` row OR a behavior described in `ir/contracts/_shared.yaml` (main_cli, frame_update_order, shared_types semantics), grep ALL of `ir/contracts/*.yaml` for the prior value/phrase and update every match in the same commit — not just the shard adjacent to the spec edit. Mechanical recipe: `grep -nE '<old-value-or-phrase>' ir/contracts/*.yaml`. The 2026-05-11 release regen surfaced two drifts surviving prior commits because only the adjacent shard was updated: `raycaster.yaml § HORIZON_Y` still read `WINDOW_HEIGHT / 2` after specs/25 introduced the HUD strip carve-out, and `_shared.yaml § main_cli § render_mode` had three "crosshair" references after commit `bb6115d` dropped the on-view crosshair from `specs/45` and `renderer.yaml`. The Reconciler-side complement is `tooling/agents/reconciler.md § Step 3` Cross-shard staleness check, but Reconciler runs after Coder — catching it here saves a regen pass.
...
- [ ] **Enumerable feature sets pinned in IR contract.** When a spec lists an enumerable set of features implemented by a single function (e.g. `specs/30 § Available Fields` → `autopilot::get_field_value`, `specs/30 § Available Targets` → `autopilot::resolve_named_target`, `specs/35 § CLI` → `main.rs::parse_args` flag set, `Archetype` variants → `enemy_logic::archetype_stats`), the corresponding `ir/contracts/<module>.yaml` MUST pin the full enumerable set explicitly in the function's contract block — not just "the function exists, see spec". Without an explicit pin the next Coder reads the spec, ships a subset, and the silent drop is invisible to Reconciler unless it manually cross-walks the resolver against the spec. The 2026-05-14 armor regen shipped `autopilot::get_field_value` without the `player.armor` arm because `ir/contracts/autopilot.yaml § run_scenario` only said "MUST recognise the fields in spec/30" without enumerating them; Reconciler retroactively added the `Field-value resolver` subsection listing each `"field" -> AssertValue` mapping. The 2026-05-18 ammo-split regen shipped `autopilot::resolve_named_target` without the `"pickup_shells"` arm because `ir/contracts/autopilot.yaml § bot_step § Decision policy step 1` described the field-value resolver in detail but left named-target resolution implicit; Reconciler retroactively added the `Named-target resolver` enumeration. The pattern recurs once per spec resolver that enumerates targets but isn't pinned in the contract — Architect should sweep every `Available X` / `Supported X` table at design time, not wait for Reconciler to backfill. Pattern: every spec's `Available X` / `Supported X` table needs a matching contract block that names each entry as a `MUST` requirement.
