...
- **There is no baseline to "carry from".** Do NOT use the phrase "carried
  ...
  **Bad vs Good rewrite.** The 2026-05-22 PR #84 regen failed the
  orchestrator's content-gate on the two phrases below — eighth
  consecutive recurrence, and the first one the gate (added in PR #82)
  actually caught at submit time. Use these as a literal template before
  writing the report:

  - Bad: `All 14 modules in scope. 8 required code changes; 6 were already correct.`

    Good: `All 14 modules in scope. Emitted 14 .rs files from specs + IR
    this run; per-module deltas relative to generated-snapshot are listed
    under § Modules generated.`

  - Bad: `## Unchanged Modules` (as a section header).

    Good: there is no such section. Every module belongs in `### Modules
    generated` with a one-line summary of what THIS run produced (e.g.
    "input_controller: emitted poll() returning InputState built from
    winit KeyboardInput events per ir/contracts/input_controller.yaml").
    If the emitted bytes happen to be byte-identical to snapshot, append
    `(byte-identical to snapshot)` to the per-module summary line — never
    promote it to a section heading or a count.

  The transformation is mechanical: replace every "N modules already
  correct / unchanged / carried" with the explicit list of what was
  emitted, and replace "Unchanged Modules" headers with per-module
  summaries under the existing `### Modules generated` block.
...
- [ ] **Before submitting `artifacts/coder_report.md`, self-grep for forbidden release-style framing phrases.** Run `grep -nE 'Unchanged|already correct|carried from|carry-forward|pre-existing|no changes needed|baseline was correct|no changes required' artifacts/coder_report.md` — every hit is non-compliance per § Release regeneration mode and MUST be rewritten to describe what was emitted this run. The rule applies whenever `--mode release` is the run mode (the only mode the orchestrator accepts), INCLUDING `pr.yml` runs whose `## Scope override` lists all 14 modules because a global-trigger spec was touched (`specs/00_project_goal.md`, `_shared.yaml`, etc.) — `partial_regen.py` forces full scope and the regen is functionally release-style. The same forbidden phrasing has now shipped in EIGHT consecutive release-style regens (2026-05-11, -12, -14, -15 x2, -16 x2, -22) despite explicit prompt prohibition; orchestrator's content-gate (PR #82) catches it at submit and fails the run, Reconciler's grep catches the same class after the fact, but Coder-side self-grep BEFORE submit is the cheaper enforcement point. See § Release regeneration mode for the Bad-vs-Good rewrite template — apply that transformation when the grep hits, do not hand-edit only the matched word. This is parallel to the `#[allow(dead_code)]` and `### Invented constants` self-greps above and runs in one command before submit.
